...
export const pathApi = {
 ...
 create: (path: Omit<Path, 'id' | 'created_at' | 'updated_at' | 'status'>) => poster<{ path: Path }, Omit<Path, 'id' | 'created_at' | 'updated_at' | 'status'>>('/paths', path).then(res => res.path),
 update: (path: Partial<Path> & { id: string }) => putter<{ path: Path }, Partial<Path>>(`/paths/${path.id}`, path).then(res => res.path),
...
export const useCreatePath = () => {
 const queryClient = useQueryClient();
 return useMutation<Path, Error, Omit<Path, 'id' | 'created_at' | 'updated_at' | 'status'>>({
...
export const useUpdatePath = () => {
 const queryClient = useQueryClient();
 return useMutation<Path, Error, Partial<Path> & { id: string }>({
