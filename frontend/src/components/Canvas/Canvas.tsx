...
import type { Node, Path, Position, NodeID, CreatePathRequest } from '@/types'
...
const handleNodeClick = useCallback((nodeId: NodeID) => {
  ...
  if (editMode === 'delete') {
    ...
    if (!isConnecting) {
      ...
      const newPath: CreatePathRequest = {
        name: `Path ${paths.length + 1}`,
        type: 'normal',
        start_node_id: connectingNodeId,
        end_node_id: nodeId,
        weight: 1,
        metadata: {},
      }
      // addPath(newPath) // Add to Zustand store - API will handle creation
...
return (
  ...
  >
    <Layer>
      {/* Render Paths */}
      {paths.map((path) => {
        const fromNode = nodes.find((n) => n.id === path.start_node_id)
        const toNode = nodes.find((n) => n.id === path.end_node_id)
